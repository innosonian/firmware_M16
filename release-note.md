...
## ver 1.1.53 (2026.06.04)
- `3c6f9cd`

### 문제 해결
- OTA 업데이트 완료 후 재부팅 과정이 더 안정적으로 진행되도록 개선.
- 배터리 상태 표시가 순간적인 측정값 변화에 흔들리지 않도록 개선.
- BLE 이벤트 알림이 짧은 시간에 반복 전송되지 않도록 조정하여 연결 중 동작 안정성 개선.

### 변경/추가
- 훈련 중 BLE 연결이 일시적으로 끊겨도 훈련 데이터를 내부에 저장하고, 훈련 종료 후 앱으로 다시 전송할 수 있는 기능 추가.

---

## ver 1.0.247 (2026.05.18)
- `1150cea`
